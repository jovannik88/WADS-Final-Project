...
describe("lib/api-helpers — verifySession", () => {
  ...
  test("returns decoded token when cookie is valid", async () => {
    mockedAdminAuth.verifySessionCookie.mockResolvedValueOnce(MOCK_USER);
    const req = makeReq("http://localhost/api/tasks", { cookie: "valid-cookie" });
    const result = await verifySession(req);
    expect(result).toEqual(MOCK_USER);
  });

  test("returns null when cookie verification throws", async () => {
    mockedAdminAuth.verifySessionCookie.mockRejectedValueOnce(new Error("expired"));
    const req = makeReq("http://localhost/api/tasks", { cookie: "bad-cookie" });
    const result = await verifySession(req);
    expect(result).toBeNull();
  });
});
...
describe("GET /api/tasks", () => {
  test("returns 401 when not authenticated", async () => {
    mockedAdminAuth.verifySessionCookie.mockRejectedValueOnce(new Error("no session"));
    const req = makeReq("http://localhost/api/tasks");
